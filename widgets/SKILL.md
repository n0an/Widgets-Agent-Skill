...
- For a multicolor image or icon that the accent group would crush into a flat silhouette, apply `.luminanceToAlpha()` so bright pixels become opaque and dark pixels transparent - this preserves the shape's internal detail as a clean monochrome glyph in tinted/accented contexts. Pair it with a soft gradient background and `.widgetAccentable()`. (See `references/rendering-modes-and-tinting.md`.)
